...
const App = ({navigation}) => {
  ...
  const [appointmentList, setAppointmentList] = useState([
    ...
    const handleAccept = async (appointID) => {
      try {
        const userData = {
        ...
        const response = await fetch(`${ip}Nurse/UpdateAppointmentStatus`, {
          method: 'PUT',
          headers: {
    ...
    const handleDecline = async (appointID) => {
      ...
      try {
        ...
        if (response.ok) {
          // Appointment status updated successfully, you can refresh the data if needed
          fetchPatientProfile();
        } else {
          console.error('Failed to update appointment status.');
        }
    ...
    const fetchPatientProfile = async () => {
      console.log('NurseID is:', NurseID);
      try {
        const response = await fetch(`${ip}Nurse/GetNotifications?NurseID=${NurseID}`);
      ...
      <View style={styles.mainContainer}>
        ...
        <View style={styles.imageBellContainer}>
          ...
          <View style={styles.headerTextContainer}>
            <Text style={styles.headerText}>Nurse Notifications</Text>
          </View>
  ...
  <View key={appointment.AppointmentID} style={styles.appointmentListItem}>
    <View style={styles.appointmentDetailsBox}>
      ...
      {appointment.Status !== 'Accepted' && appointment.Status !== 'Declined' && appointment.Status !== 'Completed' && (
        <View style={styles.acceptDeclineContainer}>
          <TouchableOpacity
            style={styles.acceptButton}
            onPress={() => handleAccept(appointment.AppointmentID)}
          >
            <Text style={styles.buttonText}>Accept</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.declineButton}
            onPress={() => handleDecline(appointment.AppointmentID)}
          >
