      ...
      if (appointmentsArray.length > 0) {
        return (
          <View style={styles.appointmentHistoryScrollView}>
            {appointmentsArray.map(([appointmentId, appointment]) => (
 ...
 new Date(appointment.Historydate) < new Date() && (
    <TouchableOpacity
      ...
      onPress={() => {
                        ...
                        navigation.navigate('genralrating', {
  ...
  {new Date(appointment.Historydate) < new Date() ? 'True' : 'False'}
        ...
        return <Text style={{ color: 'red', textAlign: 'center', fontSize: 18 }}>Haven't any History!</Text>;
      }
    } else {
      return <Text style={{ fontSize: 18, color: 'red', textAlign: 'center' }}>Haven't Any Appointment History!</Text>;
    }
  };
  ...
  const fetchData = async () => {
    try {
      const apiUrl = `${ip}Nurse/GetAppointmentHistory?NurseID=${UserID}&AppointDate=${selectedDate.toLocaleDateString()}&Status=${datePickerMode}`;
      console.log(apiUrl);
