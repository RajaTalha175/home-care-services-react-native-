...
const App = () => {
  ...
  useEffect(() => {
    const fetchNurseSchedules = async () => {
      try {
        const response = await fetch(`http://192.168.224.21/HomeCareServices/api/Nurse/GetNurseSchedules?nurseID=${UserID}`);
        if (response.ok) {
  ...
  const UpdateSelection = async() => {
    ...
    try {
      const response = await fetch('http://192.168.224.21/HomeCareServices/api/Nurse/UpdateNurseSchedules', {
        method: 'PUT',
        headers: {
