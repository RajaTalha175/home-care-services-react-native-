...
const ServiceList = () => {
  ...
  const handleSavebutton = async () => {
    console.log('Sending NurseID:', nurseDetail?.NurseID);
    if (selectedServices.length > 0) {
      ...
      try {
        const response = await fetch(`${ip}Nurse/AddNurseService`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          navigation.navigate('nurseschedule', { UserID: UserID });
        } else {
    ...
    >
      <View style={styles.container}>
        {loading ? (
          ...
          <>
            <Text style={styles.title}>Nurse Services Selection</Text>
