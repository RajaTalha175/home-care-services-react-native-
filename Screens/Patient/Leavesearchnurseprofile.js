...
const NurseProfile = ({ navigation }) => {
  ...
  useEffect(() => {
    if (!nurses || !AppointDetail) {
      console.error('1. nurses or AppointDetail is null');
      navigation.goBack();
    } else {
      
  
      console.log('////////////////');
      console.log('Recived Nurse is:', nurses);

      console.log('Recived Data is:', AppointDetail); // Log the entire route params
    }
  }, [nurses, AppointDetail, navigation]);
  useEffect(() => {
    const fetchNurseServicePackages = async () => {
      try {
        const response = await fetch(`${ip}Search/GetNurseServicePakages?NurseID=${nurses.NurseID}&ServiceName=${AppointDetail.ServiceName}`);
  ...
  }, [nurses, AppointDetail, ip]);
  
  useEffect(() => {
    const fetchNurseSchedules = async () => {
      try {
        
        const datefrom = new Date(AppointDetail.AppointDetail.leavedatefrom);
const dateto = new Date(AppointDetail.AppointDetail.leavedateto);
const formattedDateFrom = datefrom.toLocaleDateString();
const formattedDateTo = dateto.toLocaleDateString();
        console.log('LeaveDateTime: ',nurses.NurseID,formattedDateFrom,formattedDateTo);
        const apiUrl = `${ip}Search/NurseScheduleShow?NurseID=${nurses.NurseID}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`;
...
const formattedDateTo = dateto.toLocaleDateString();
      const userData = {
       PatientID:AppointDetail.PatientID,
        NurseleaveID:AppointDetail.AppointDetail.NurseLeaveID,
  ...
  console.log('Sending Data is:',userData);
      const response = await fetch(`${ip}Patient/AddAppointnment`, {
        method: 'POST',
        headers: {
        ...
        {nurses && (
          <View style={styles.profileContainer}>
            ...
            <View style={styles.imageContainer}>
              <Image
                 source={{ uri: `data:image/png;base64,${nurses.Baseimage64}` }}
                style={styles.image}
              />
            </View>
            ...
            <View style={styles.detailsContainer}>
              <Text style={styles.name}>{nurses.FullName}</Text>
              <Text style={styles.details}>PhoneNo: {nurses.PhoneNo}</Text>
            </View>
          </View>
        )}
